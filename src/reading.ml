...
and read_before root id =
  ...
  let children_rep =
    let rec parse_children root acc =
      let next =
        (root##connectedEdges (Js.string ".before"))##targets##not root
      in
...
and read_tree root =
  ...
  if Array.mem classes "prime" ~equal:String.equal then read_prime root id
  else if Array.mem classes "before" ~equal:String.equal then
    read_before (Js.Unsafe.coerce root) id
  else
    let all_edges = (Js.Unsafe.coerce root)##connectedEdges in
    let edges_to = all_edges##not (all_edges##edges (Js.string ".before")) in
    let successors =
      edges_to##targets##not root |> Js.to_array |> Array.to_list
    in
    if List.is_empty successors then
      read_atom (Js.Unsafe.coerce root) id label_string
    else
      let tl = List.map successors ~f:read_tree in
      let connective =
        if String.equal label_string "⊗" then Tree.Tensor tl
        else if String.equal label_string "⅋" then Tree.Par tl
        else raise_s (Sexp.message "error" [])
      in
      { Tree.connective; id }
