...
let serialize_graph ?directed graph =
  ...
  let edges =
    ...
    | Some bool ->
        if bool then `List (from_edges graph.edges @ from_edges graph.edges_from)
        else `List (from_edges graph.edges)
  in
...
let rec serialized_nodes_and_edges (tree : Tree.tree) =
  ...
  let new_node =
    ...
    | Some id_graph -> `Assoc (("graph", from_id_graph id_graph) :: node_base)
  in
  match successors with
  | [] -> ([ new_node ], [])
  | l ->
      let nodes, edges =
        List.map l ~f:serialized_nodes_and_edges |> Stdlib.List.split
      in
      let node = new_node :: List.concat nodes in
      let new_edges =
        List.map successors ~f:(fun (t : Tree.tree) ->
